...
class SineWave(Node):
    def __init__(self):
        ...
        # frequency = 5,40,18 per test sinusoidali amplitude 1.0
        n = 10.0
        f = 20.0 
        self.T = 1 / f
        self.time = self.T * n
    ...
    def getup_callback(self):
        ...
        if self.i < WARMUP_ZONE:
            ...
            # sin_value = (2.0 * math.sin(self.w * self.t))
            self.torque = np.array([self.amplitude * math.sin(self.w * self.t)])
...
def main(args=None):
    ...
    sinewave_test = SineWave()
    rclpy.spin(sinewave_test)
    sinewave_test.destroy_node()
    rclpy.shutdown()
